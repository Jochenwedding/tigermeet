...
export class TigerRoom {
  ...
  onMessage(ws, raw) {
    ...
    if (msg.type === 'join') {
      ...
      this.broadcast({ type: 'event', text: `${name} joined TigerGames 🐯` });
      return;
    }
    if (msg.type === 'input') {
      const p = this.players.get(ws); if (!p) return;
      this.inputs.set(p.id, { angle: Number(msg.angle) || 0, boost: !!msg.boost });
      return;
    }
    if (msg.type === 'respawn') {
      const old = this.players.get(ws); if (!old) return;
  ...
  createPlayer(id, name) {
    ...
    const p = { id, name, x, y, angle: a, color: COLORS[Math.floor(Math.random()*COLORS.length)], score: 0, alive: true, body: [], radius: 13 };
    for (let i=0;i<22;i++) p.body.push({ x: x - Math.cos(a)*i*11, y: y - Math.sin(a)*i*11 });
    return p;
  }
  ...
  tick() {
    if (this.players.size === 0) { clearInterval(this.tickHandle); this.tickHandle = null; return; }
    const now = Date.now();
    const dt = Math.min((now - this.lastTick) / 1000, 0.06);
    this.lastTick = now;
    this.seedFood();
    for (const p of this.players.values()) if (p.alive) this.updatePlayer(p, dt);
    this.handleEating();
    this.handleCrashes();
    this.broadcastState();
  }
  ...
  updatePlayer(p, dt) {
    ...
    if (canBoost && Math.random() < 0.52) {
      ...
      if (tail) this.food.push({ x: tail.x, y: tail.y, r: 6, v: 1.2 });
  ...
  kill(dead, killer) {
    ...
    for (let i=0;i<dead.body.length;i+=2) {
      const b = dead.body[i];
      this.food.push({ x: b.x + rand(-18,18), y: b.y + rand(-18,18), r: rand(5,9), v: rand(1.4,3.5) });
    }
    this.broadcast({ type: 'event', text: `${killer.name} at ${dead.name} op 🐯` });
  }

  seedFood() {
    while (this.food.length < FOOD_TARGET) this.food.push({ x:rand(40,WORLD-40), y:rand(40,WORLD-40), r:rand(4,7), v:rand(0.7,2.1) });
  }

  broadcastState() {
    const players = [...this.players.values()].map(p => ({ id:p.id, name:p.name, x:p.x, y:p.y, color:p.color, score:p.score, alive:p.alive, radius:p.radius, len:p.body.length, body:p.body.filter((_,i)=>i%2===0) }));
    const msg = JSON.stringify({ type:'state', players, food:this.food.slice(0,700), top5:this.top5 });
    this.broadcastRaw(msg);
  }

  broadcast(obj) { this.broadcastRaw(JSON.stringify(obj)); }
  broadcastRaw(msg) {
  ...
  async saveTop(name, score) {
    ...
    this.top5.push({ name, score, at: Date.now() });
...
function safeName(v){ return String(v).replace(/[<>"&]/g,'').trim().slice(0,16) || 'Tiger'; }
