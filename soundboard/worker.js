...
if (url.pathname === "/api/soundboard-log" && request.method === "POST") {
  ...
  await env.LOGS.put(
    "soundboard_logs",
    JSON.stringify(oldLogs.slice(0, 250))
  );
...
if (url.pathname === "/admin/soundboard-logs") {
  ...
  const rows = logs.map(log => `
    <tr>
      ...
      <td>${log.country}</td>
      <td>${log.page}</td>
      <td style="max-width:400px;overflow:hidden">${log.device}</td>
    </tr>
  `).join("");
